namespace _01.Inventory.Tests
{
    ...
    public class InventoryTests
    {
        ...
        [Test]
        public void GetByIdWorksCorrectlyReturnNull()
        {
            IWeapon weaponById = this._inventory.GetById(1000);
        ...
        [Test]
        public void RefillAmmunitionException()
        {
            int ammoToRefill = this._savedWeapon.MaxCapacity - this._savedWeapon.Ammunition;
            int expectedAmmo = this._savedWeapon.MaxCapacity;
            IWeapon fakeWeapon = new Pistol(1111, 10, 50);

            Assert.Throws<InvalidOperationException>(() => this._inventory.Refill(fakeWeapon, ammoToRefill));
            Assert.That(() => this._inventory.Refill(fakeWeapon, ammoToRefill), Throws.InvalidOperationException.With.Message.EqualTo("Weapon does not exist in inventory!"));
        }
