namespace _02.Data.Tests
{
    ...
    public class DataTests
    {
        ...
        [Test]
        public void TestGetAllByStatusWorksCorrectly()
        {
            string status = "InStore";
            var searchedType = BaseEntityStatus.InStore;
            var expectedEntities = this._data.GetAll()
                .Where(e => e.Status.Equals(searchedType))
                .ToList();
            var actualEntities = this._data.GetAllByStatus(status);

            Assert.AreEqual(expectedEntities.Count, actualEntities.Count);

            foreach (var entity in actualEntities)
            {
                Assert.AreEqual(status, entity.Status.ToString());
            }
        }
